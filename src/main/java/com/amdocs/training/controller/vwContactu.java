...
@WebServlet("/vwContactu")
public class vwContactu extends HttpServlet{
...
Connection conn = DBUtil.getConnection();
		...
		String sql = "SELECT * FROM contact WHERE user_id ='"+Person.id+"'";
...
response.getWriter().println("<button class=\"navbar-toggler\" type=\"button\" data-bs-toggle=\"collapse\" data-bs-target=\"#navbarToggleExternalContent\" aria-controls=\"navbarToggleExternalContent\" aria-expanded=\"false\" aria-label=\"Toggle navigation\">");
		...
		try {
			...
			response.getWriter().println(" <thead>");
			     ...
			     response.getWriter().println("<th scope=\"col\">Message</th>");
			     response.getWriter().println("</tr>");
			     response.getWriter().println("</thead>");
			 	...
			 	while(rs1.next()) {
			    	  response.getWriter().println("<tr>");
			    	     ...
			    	     response.getWriter().print(" <form action=\"rmContactu?usrID=");
			    	     response.getWriter().print(rs1.getString(6));
			    	     response.getWriter().print("\" method=\"post\" >");
			    	   
			    	     response.getWriter().println(" <input type=\"submit\" value=\"Remove contact\" />");
