...
@WebServlet("/vwCoursesA")
public class vwCoursesA extends HttpServlet{
			...
			response.getWriter().println(" <thead>");
			 	...
			 	while(rs1.next()) {
			    	  response.getWriter().println("<tr>");
			    	     ...
			    	     response.getWriter().print(" <form action=\"rmCourseA?usrID=");
			    	     response.getWriter().print(rs1.getString(1));
			    	     response.getWriter().print("\" method=\"post\" >");
			    	    
			    	     response.getWriter().println(" <input type=\"submit\" value=\"Remove course\" />");
