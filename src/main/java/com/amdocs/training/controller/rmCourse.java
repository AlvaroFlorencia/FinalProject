...
@WebServlet("/rmCourse")
public class rmCourse extends HttpServlet{
...
Connection conn = DBUtil.getConnection();
		...
		String sql = "SELECT * FROM course WHERE course_id IN(SELECT course_id FROM courseuser WHERE user_id ="+Person.id+")";
		     ...
		     response.getWriter().println("<tbody>");
			 	while(rs1.next()) {
			    	  response.getWriter().println("<tr>");
			    	     ...
			    	     response.getWriter().print(" <form action=\"rmiCourse?usrID=");
			    	     response.getWriter().print(rs1.getString(1));
			    	     response.getWriter().print("\" method=\"post\" >");
			    	     response.getWriter().println("<pre>");
			    	     response.getWriter().println("<pre>");
			    	     response.getWriter().println(" <input type=\"submit\" value=\"Remove a course\" />");
			    	     response.getWriter().println("</pre>");
			    	     response.getWriter().println(" </form>");
			    	     response.getWriter().println("</td>");
			    	     response.getWriter().println("</tr>");
