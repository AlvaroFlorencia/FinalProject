...
Connection conn = DBUtil.getConnection();
		...
		String sql = "SELECT * FROM user1 WHERE user_id ='"+Person.id+"'";
...
response.getWriter().println("<input type=\"submit\" value=\"View information account\" style=\"background-color:transparent;border:0;color:white\" />");
 ...
 response.getWriter().println("<button class=\"navbar-toggler\" type=\"button\" data-bs-toggle=\"collapse\" data-bs-target=\"#navbarToggleExternalContent\" aria-controls=\"navbarToggleExternalContent\" aria-expanded=\"false\" aria-label=\"Toggle navigation\">");
		...
		try {
			...
			rs1.next();
			response.getWriter().println("<h1>Bienvenido "+rs1.getString(2)+"</h1>");
			response.getWriter().println("<div class=\"text-center\">");
			response.getWriter().println("<img class=\"mb-4\" src=\""+rs1.getString(8)+"\" alt=\"\" width=\"200\" height=\"200\" style=\"border-radius: 100px;\">");
			response.getWriter().println("</div>");
			response.getWriter().println("<br>");
			response.getWriter().println("<font size=5><b >Name</b></font><font size=5> <a> "+rs1.getString(2)+"</a></font> <br>");
			response.getWriter().println("<font size=5><b >Phone number</b></font><font size=5> <a> "+rs1.getInt(3)+"</a></font> <br>");
			response.getWriter().println("<font size=5><b >Address</b></font><font size=5> <a> "+rs1.getString(5)+"</a></font><br>");
			response.getWriter().println("<font size=5><b >Email</b></font><font size=5> <a> "+rs1.getString(4)+"</a></font>");
