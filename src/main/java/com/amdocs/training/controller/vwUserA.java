...
@WebServlet("/vwUserA")
public class vwUserA extends HttpServlet{
...
response.getWriter().println("<button class=\"navbar-toggler\" type=\"button\" data-bs-toggle=\"collapse\" data-bs-target=\"#navbarToggleExternalContent\" aria-controls=\"navbarToggleExternalContent\" aria-expanded=\"false\" aria-label=\"Toggle navigation\">");
		...
		try {
			...
			response.getWriter().println(" <thead>");
			     ...
			     response.getWriter().println("<th scope=\"col\">Phone number</th>");
			     response.getWriter().println("<th scope=\"col\">Email</th>");
			     response.getWriter().println("<th scope=\"col\">Address</th>");
			     response.getWriter().println("<th scope=\"col\">Registration date</th>");
			     response.getWriter().println("<th scope=\"col\">Password</th>");
			     response.getWriter().println("<th scope=\"col\">Upload photo</th>");
			 	...
			 	while(rs1.next()) {
			    	  response.getWriter().println("<tr>");
			    	     ...
			    	     response.getWriter().println("<td>"+ rs1.getInt(3)+"</td>");
			    	     response.getWriter().println("<td>"+ rs1.getString(4)+"</td>");
			    	     response.getWriter().println("<td>"+ rs1.getString(5)+"</td>");
			    	     response.getWriter().println("<td>"+ rs1.getString(6).toString()+"</td>");
			    	     response.getWriter().println("<td>"+ rs1.getString(7)+"</td>");
			    	     response.getWriter().println("<td><img src=\""+rs1.getString(8)+"\"  width=\"52\" height=\"52\"></td>");
			    	     response.getWriter().println("<td>");
			    	     response.getWriter().print(" <form action=\"rmUserA?usrID=");
			    	     response.getWriter().print(rs1.getString(1));
			    	     response.getWriter().print("\" method=\"post\" >");
			    	     response.getWriter().println("<pre>");
			    	     response.getWriter().println("<pre>");
			    	     response.getWriter().println(" <input type=\"submit\" value=\"Remove user\" />");
			    	     response.getWriter().println("</pre>");
			    	     response.getWriter().println(" </form>");
			    	     response.getWriter().println("</td>");
			    	     response.getWriter().println("</tr>");
